...
flags = tf.app.flags
flags.DEFINE_string("name", "conditional", "Name of the experiment ")
...
pad=8
stride = 32
input_channel= 3 +FLAGS.c_dim*3

loadCheckPoint = True
load_checkpoint_dir = "best_checkpoint"
train_log_file= "train_conditional_log.txt"
vali_log_file= "vali_conditional_log.txt"
...
with tf.Session(config=config) as sess:
    ...
    for iteration in range(FLAGS.num_steps):
        ...
        train_rgb = create_rgb_with_sens( train_data, sens)
        ...
        if counter % FLAGS.display_step == 0:
            save(FLAGS.name, checkpoint_dir, iteration)
        ...
        if (counter % FLAGS.display_step == 0) :
              ...
              for k in range(validation_length):
                      ...
                      test_rgb = create_rgb_with_sens(test_data, sens)
              ...
              if temp_float < best_rgb_val_loss[0]:
                      ...
                      save(best_checkpoint_dir, counter)
